...
@Component({
  standalone: true,
  selector: 'app-product-edit-signal',
      ...
      <div class="card-body">
        <form [formGroup]="productForm" (submit)="onSubmit()">
          ...
          <div class="d-flex gap-2">
            ...
            <button routerLink="/products-signal" type="button" class="btn btn-secondary">
              Cancel
            </button>
          </div>
        </form>
...
export class ProductEditSignalComponent {
  oldProduct: Product | null | undefined = null;
  @Output() add = new EventEmitter<Product>();
  @Output() update = new EventEmitter<Product>();
  @Output() delete = new EventEmitter<number>();
  @Input() set product(product: Product | null | undefined) {
    // Reset form when component initializes or when explicitly passed null/undefined
    if (!product) {
    ...
    // Only set form values if we have a product with an ID (editing mode)
    if (product.id !== 0) {
  ...
  onSubmit() {
    ...
    // If we have an oldProduct with an ID, it's an update operation
    if (this.oldProduct && this.oldProduct.id > 0) {
